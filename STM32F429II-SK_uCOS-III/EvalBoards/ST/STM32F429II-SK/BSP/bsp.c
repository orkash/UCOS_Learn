...
#define  BSP_GPIOD_LED1                        GPIO_Pin_12
...
static void  BSP_LED_Init()
{
    ...
    gpio_init.GPIO_Pin   = BSP_GPIOD_LED1;
    ...
    BSP_PeriphEn(BSP_PERIPH_ID_GPIOD);
    gpio_init.GPIO_Pin   = BSP_GPIOD_LED2;
    gpio_init.GPIO_Mode  = GPIO_Mode_OUT;
    gpio_init.GPIO_OType = GPIO_OType_PP;
    gpio_init.GPIO_PuPd  = GPIO_PuPd_UP;
    gpio_init.GPIO_Speed = GPIO_Speed_50MHz;
    ...
    gpio_init.GPIO_Pin = (BSP_GPIOD_LED3 | BSP_GPIOD_LED4);
    gpio_init.GPIO_Mode  = GPIO_Mode_OUT;
    gpio_init.GPIO_OType = GPIO_OType_PP;
    gpio_init.GPIO_PuPd  = GPIO_PuPd_UP;
    gpio_init.GPIO_Speed = GPIO_Speed_50MHz;
...
void  BSP_LED_On (CPU_INT08U led)
{
    switch (led) {
        ...
        case 4u:
             GPIO_ResetBits(GPIOE, BSP_GPIOD_LED4);
             break;
...
void  BSP_LED_Toggle (CPU_INT08U  led)
{
    ...
    switch (led) {
        case 0u:
             pins  =  GPIO_ReadOutputData(GPIOD);               /* Read data (current state of output pins) in GPIOA    */
             pins ^=  BSP_GPIOD_LED1;                           /* Tells which pins (other than pin for LED1) are ON    */
             GPIO_SetBits(  GPIOD,   pins  &  BSP_GPIOD_LED1);  /* If there are any,turn them OFF and leave LED1 ON     */
             GPIO_ResetBits(GPIOD, (~pins) &  BSP_GPIOD_LED1);

             pins  =  GPIO_ReadOutputData(GPIOD);
             pins ^=  BSP_GPIOD_LED2;
             GPIO_SetBits(  GPIOD,   pins  &  BSP_GPIOD_LED2);
             GPIO_ResetBits(GPIOD, (~pins) &  BSP_GPIOD_LED2);

             pins  =  GPIO_ReadOutputData(GPIOD);
             pins ^= (BSP_GPIOD_LED3       |  BSP_GPIOD_LED4);
             GPIO_SetBits(  GPIOD,   pins  & (BSP_GPIOD_LED3  |  BSP_GPIOD_LED4));
             GPIO_ResetBits(GPIOD, (~pins) & (BSP_GPIOD_LED3  |  BSP_GPIOD_LED4));
             break;
        ...
        case 1u:
             pins  = GPIO_ReadOutputData(GPIOD);
             pins ^= BSP_GPIOD_LED1;
             GPIO_SetBits(  GPIOD,   pins   &  BSP_GPIOD_LED1);
             GPIO_ResetBits(GPIOD, (~pins)  &  BSP_GPIOD_LED1);
             break;
        ...
        case 3u:
             pins  = GPIO_ReadOutputData(GPIOD);
             pins ^= BSP_GPIOD_LED3;
             GPIO_SetBits(  GPIOD,   pins   &  BSP_GPIOD_LED3);
             GPIO_ResetBits(GPIOD, (~pins)  &  BSP_GPIOD_LED3);
             break;
